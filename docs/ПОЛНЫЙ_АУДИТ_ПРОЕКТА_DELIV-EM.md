...
**✅ ИСПРАВЛЕНО [2026-05-05]**
- Добавлены API routes с `unstable_cache`:
  - `/api/couriers` — кэширует список курьеров на 5 минут
  - `/api/ratings/batch` — кэширует рейтинги на 10 минут
  - `/api/tasks/completed` — кэширует завершённые задачи на 10 минут
- Обновлен `CouriersList.tsx` для использования кэшированных API вместо прямых Supabase запросов
- Города уже кэшированы как константа в `lib/types.ts`
