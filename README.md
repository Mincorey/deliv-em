...
Реализована полная система аудит логирования для отслеживания всех изменений в критичных таблицах БД. Создана новая таблица `audit_log` с индексами для быстрого поиска по table_name, record_id, user_id, operation и created_at. Реализована PL/pgSQL функция `audit_trigger_func()` которая автоматически логирует все INSERT, UPDATE и DELETE операции с сохранением старых и новых значений. Триггеры подключены к критичным таблицам: transactions (финансовые операции), ratings (рейтинги), tasks (заказы) и profiles (профили пользователей). Создан сервис `lib/services/audit.ts` с функциями для просмотра аудит логов, фильтрации по таблице/пользователю/дате, отслеживания истории записей и анализа изменений. Добавлен API маршрут `/api/audit` для запроса логов (типы запроса: all, record, user, table, stats). Создана подробная документация в `docs/AUDIT_LOG_GUIDE.md` с примерами использования и best practices. RLS политики защищают аудит логи от несанкционированного доступа.
---
Implemented complete audit logging system for tracking all changes in critical database tables. Created new `audit_log` table with indexes for fast searching by table_name, record_id, user_id, operation, and created_at. Implemented PL/pgSQL function `audit_trigger_func()` that automatically logs all INSERT, UPDATE, and DELETE operations with old and new values saved. Triggers connected to critical tables: transactions (financial operations), ratings (ratings), tasks (orders), and profiles (user profiles). Created `lib/services/audit.ts` service with functions to view audit logs, filter by table/user/date, track record history, and analyze changes. Added `/api/audit` route for querying logs (query types: all, record, user, table, stats). Created detailed documentation in `docs/AUDIT_LOG_GUIDE.md` with usage examples and best practices. RLS policies protect audit logs from unauthorized access.
==============

## 2026-05-05 (Documentation Reorganization)
Все файлы документации перемещены в новую папку `docs/`. Создан файл `docs/README.md` как индекс всей документации с быстрой навигацией по всем гайдам. Добавлен файл `docs/CONTRIBUTING.md` с инструкциями для разработчиков по добавлению новой документации. Главный `README.md` обновлен со ссылкой на папку docs в начало документа. Впредь все файлы документации будут размещаться в папке `docs/` для удобства и централизации.
---
All documentation files moved to new `docs/` folder. Created `docs/README.md` as documentation index with quick navigation. Added `docs/CONTRIBUTING.md` with guidelines for developers to add new documentation. Main `README.md` updated with link to docs folder at the beginning. Going forward, all documentation files will be placed in `docs/` folder for convenience and centralization.
==============

## 2026-05-05 (Soft Delete System Implementation)
Реализована полная система мягкого удаления данных с возможностью восстановления для критичных таблиц. Создана SQL миграция `11_add_soft_delete.sql` которая добавляет колонку `deleted_at` в таблицы profiles, tasks, ratings, messages и feedback. Реализованы RLS политики которые автоматически скрывают удалённые записи из обычных запросов. Созданы вспомогательные функции для мягкого удаления, восстановления и окончательного удаления старых данных. Реализован TypeScript сервис `lib/services/softDelete.ts` с 10+ функциями для работы с удаленными данными, включая статистику, массовые операции и GDPR cleanup. Добавлен API маршрут `/api/soft-delete` для REST операций. Все удаления автоматически логируются в аудит систему. Создана подробная документация в `docs/SOFT_DELETE_GUIDE.md` с примерами и лучшими практиками.
---
Implemented complete soft delete system for data recovery capability on critical tables. Created SQL migration `11_add_soft_delete.sql` that adds `deleted_at` column to profiles, tasks, ratings, messages, and feedback tables. Implemented RLS policies that automatically hide deleted records from regular queries. Created helper functions for soft delete, restore, and permanent deletion of old data. Implemented TypeScript service `lib/services/softDelete.ts` with 10+ functions for managing deleted data, including statistics, bulk operations, and GDPR cleanup. Added `/api/soft-delete` route for REST operations. All deletions automatically logged to audit system. Created detailed documentation in `docs/SOFT_DELETE_GUIDE.md` with examples and best practices.
==============

## 2026-05-05 (Auth Result Type Improvements)
Исправлена типизация результатов функций аутентификации для большей ясности кода. Обновлены типы в `src/app/(auth)/auth/actions.ts`: вместо неявного типа `{ error?: string }` (где успех представлялся пустым объектом) использован discriminated union `AuthResult` с явным флагом `ok: true | false`. Функции `loginAction()` и `registerAction()` теперь возвращают `{ ok: true }` при успехе и `{ ok: false; error: string }` при ошибке. Обновлены компоненты-потребители: `src/app/(auth)/auth/page.tsx` и `src/app/_landing/LandingAuth.tsx` для проверки `!result.ok` вместо `result.error`, что делает логику более явной и улучшает TypeScript type narrowing.
---
Fixed result type annotations in authentication functions for better code clarity. Updated types in `src/app/(auth)/auth/actions.ts`: replaced implicit `{ error?: string }` type (where success was represented as empty object) with discriminated union `AuthResult` with explicit `ok: true | false` flag. Functions `loginAction()` and `registerAction()` now return `{ ok: true }` on success and `{ ok: false; error: string }` on error. Updated consuming components: `src/app/(auth)/auth/page.tsx` and `src/app/_landing/LandingAuth.tsx` to check `!result.ok` instead of `result.error`, making logic more explicit and improving TypeScript type narrowing.
==============

## 2026-05-05 (Caching Layer for Couriers, Ratings, Tasks)
Добавлена инфраструктура кэширования для оптимизации производительности. Созданы три новые API routes с использованием Next.js `unstable_cache`: `/api/couriers` (кэширует список курьеров на 5 минут), `/api/ratings/batch` (кэширует рейтинги на 10 минут), `/api/tasks/completed` (кэширует завершённые задачи на 10 минут). Обновлен компонент `CouriersList.tsx` для использования кэшированных API routes вместо прямых Supabase запросов, что снижает нагрузку на БД и улучшает время загрузки страницы. Системы кэширования использует встроенный Next.js механизм с автоматической переоценкой по истечении времени и поддержкой `revalidatePath` для инвалидации кэша при обновлении данных.
---
